...
type MongoTransfer = Omit<Transfer, 'id' | 'fromAccountId' | 'toAccountId'> & {
  _id: string;
  fromAccountId: string;
  toAccountId?: string;
};
type MongoPayee = Omit<Payee, 'id' | 'userId'> & { _id: string; userId: string };
type MongoBillPayment = Omit<BillPayment, 'id' | 'userId' | 'payeeId' | 'accountId'> & {
  _id: string;
  userId: string;
  payeeId: string;
  accountId: string;
};
type MongoCard = Omit<Card, 'id' | 'userId' | 'accountId'> & {
  _id: string;
  userId: string;
  accountId: string;
};
...
export class PostgresStorage implements IStorage {
  ...
  async createUser(
    insertUser: Omit<InsertUser, 'password'> & { password: string }
  ): Promise<MongoUser> {
  ...
  async createAccount(
    insertAccount: Omit<InsertAccount, 'userId'> & { userId: string }
  ...
  async getTransactionsByAccountId(
    ...
    return result.map(transaction =>
      this.pgToMongo<Transaction, MongoTransaction>(transaction, undefined, 'accountId')
    );
  }
  ...
  async createTransaction(
    insertTransaction: Omit<InsertTransaction, 'accountId'> & { accountId: string }
  ): Promise<MongoTransaction> {
    const result = await db
  ...
  async createTransfer(
    insertTransfer: Omit<InsertTransfer, 'fromAccountId' | 'toAccountId'> & {
      fromAccountId: string;
      toAccountId?: string;
    }
  ): Promise<MongoTransfer> {
    const result = await db
  ...
  async updateTransfer(
    id: string,
    updateData: Partial<MongoTransfer>
  ): Promise<MongoTransfer | null> {
    ...
    const {
      id: resultId,
      fromAccountId: resultFromAccountId,
      toAccountId: resultToAccountId,
      ...rest
    } = result[0];
    return {
  ...
  async createPayee(
    insertPayee: Omit<InsertPayee, 'userId'> & { userId: string }
  ): Promise<MongoPayee> {
    const result = await db
  ...
  async createBillPayment(
    insertBillPayment: Omit<InsertBillPayment, 'userId' | 'payeeId' | 'accountId'> & {
      userId: string;
      payeeId: string;
      accountId: string;
    }
  ): Promise<MongoBillPayment> {
    const result = await db
  ...
  async createCard(
    insertCard: Omit<InsertCard, 'userId' | 'accountId'> & { userId: string; accountId: string }
  ): Promise<MongoCard> {
    const result = await db
      .insert(schema.cards)
      .values({
  ...
  async getCryptoHoldingsByUserId(userId: string): Promise<MongoCryptoHolding[]> {
    ...
    return result.map(holding =>
      this.pgToMongo<CryptoHolding, MongoCryptoHolding>(holding, 'userId')
    );
  }
  ...
  async createCryptoHolding(
    insertCryptoHolding: Omit<InsertCryptoHolding, 'userId'> & { userId: string }
  ): Promise<MongoCryptoHolding> {
    const result = await db
  ...
  async updateCryptoHolding(
    id: string,
    updateData: Partial<MongoCryptoHolding>
  ): Promise<MongoCryptoHolding | null> {
    const { _id, userId, ...pgUpdateData } = updateData;
  ...
  async getNotificationsByUserId(userId: string): Promise<MongoNotification[]> {
    ...
    return result.map(notification =>
      this.pgToMongo<Notification, MongoNotification>(notification, 'userId')
    );
  }

  async createNotification(
    insertNotification: Omit<InsertNotification, 'userId'> & { userId: string }
  ): Promise<MongoNotification> {
    const result = await db
  ...
  async updateNotification(
    id: string,
    updateData: Partial<MongoNotification>
  ): Promise<MongoNotification | null> {
    const { _id, userId, ...pgUpdateData } = updateData;
    const updateValues: any = { ...pgUpdateData };
