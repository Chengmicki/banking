...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/transfers', authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (fromAccount.transactionsBlocked || fromAccount.outgoingBlocked) {
        return res
          .status(403)
          .json({ message: 'Outgoing transactions are blocked for this account' });
      }
      ...
      if (transferData.toAccountId) {
        ...
        if (toAccount && (toAccount.transactionsBlocked || toAccount.incomingBlocked)) {
          return res
            .status(403)
            .json({ message: 'Incoming transactions are blocked for the destination account' });
        }
      }
    ...
    try {
      ...
      if (existingHolding) {
        ...
        const newAverageCost =
          (parseFloat(existingHolding.amount) * parseFloat(existingHolding.averageCost) +
            totalCost) /
          newAmount;
  ...
  app.put(
    '/api/admin/accounts/:id/block-transactions',
    authenticateAdmin,
    requirePermission('manage_accounts'),
    async (req: AdminAuthRequest, res) => {
      try {
  ...
  app.get(
    '/api/admin/cards',
    authenticateAdmin,
    requirePermission('manage_cards'),
    async (req: AdminAuthRequest, res) => {
      try {
        const cards = await storage.getAllCards();
        res.json(cards);
  ...
  app.get(
    '/api/admin/crypto',
    authenticateAdmin,
    requirePermission('manage_crypto'),
    async (req: AdminAuthRequest, res) => {
      try {
        const cryptoHoldings = await storage.getAllCryptoHoldings();
        res.json(cryptoHoldings);
