...
export async function seedAdmin() {
  try {
    ...
    const existingAdmin = await db.select().from(schema.admins).where(eq(schema.admins.username, "admin")).limit(1);
...
async function seedSampleData() {
  try {
    ...
    console.log("Sample accounts created:", { checkingAccount: checkingAccount[0], savingsAccount: savingsAccount[0] });
    ...
    const cryptoHoldings = await db.insert(schema.cryptoHoldings).values([
      {
        userId: user[0].id,
        symbol: "BTC",
        name: "Bitcoin",
